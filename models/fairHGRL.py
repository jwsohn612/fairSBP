...
class fairHGRL(): 
    ...
    def define_networks(self):
        self.f = self.define_classifier() # X -> f(X)
        self.e = self.define_hgr_network(self.adim) # A -> h(A)
        self.g = self.define_hgr_network(self.ldim)
        ...
        if self.fair_type == "eo":
            self.e2 = self.define_hgr_network(1) # A -> h(A)
            self.g2 = self.define_hgr_network(self.ldim)
    ...
    def train_minimization_step(self, X,A,Y):
        ...
        with tf.GradientTape(persistent=True) as n:
            ...
            C = self.fit_loss(Y, self.T(self.f(X, training = True),training=True))*(1-self.plam) + self.plam * self.evaluate_soft_HGR(self.e, self.g, X,A)

        fgrad = n.gradient(C, self.f.trainable_variables)
        Tgrad = n.gradient(C, self.T.trainable_variables)
    ...
    def train_minimization_step_eo(self, X,A,Y):
        ...
        with tf.GradientTape(persistent=True) as n:
            D = tf.multiply(A,Y)
            C = self.fit_loss(Y,self.T(self.f(X, training = True),training=True))*(1-self.plam) + self.plam *(self.evaluate_soft_HGR(self.e, self.g, X,D)-self.evaluate_soft_HGR(self.e2,self.g2,X,Y))
    ...
    def evaluate(self, X):
        return self.T(self.f(X, training = False), training=False)