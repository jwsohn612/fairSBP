...
class fairSBPL(): 
    ...
    def define_discriminator(self):
        x_ = layers.Input(shape = (self.ldim,)) 
    ...
    def train_fair_classifier(self, X, A, Y):
        with tf.GradientTape(persistent=True) as t:
            ...
            main_loss = self.fit_loss(Y, self.H(YhatA, training = True))
            neural = self.neural_loss(dYhatA, dYhatT)
            total_loss = main_loss*(1 - self.plam) - neural * self.plam  

        grad = t.gradient(total_loss, self.h.trainable_variables)
        gradH = t.gradient(total_loss, self.H.trainable_variables)
        self.h_optim.apply_gradients(zip(grad, self.h.trainable_variables))
        self.H_optim.apply_gradients(zip(gradH, self.H.trainable_variables))
        
        return main_loss

    def generate_random_sensi(self, A):
        gen = tf.random.shuffle(A)
        #gen = tf.random.uniform(A.shape, minval=0,maxval=1)
        #gen = tf.cast(gen > 0.5, dtype=tf.float32)
        return gen
...
#             ref = tf.random.normal(shape=A.shape, mean=0.5, stddev=2)
...
#         ref = tf.random.normal(shape=A.shape, mean=0.5, stddev=2)
        ...
        dYhatT = self.da(self.pD([Y, A], training = False))
        neural = self.neural_loss(dYhatA, dYhatT)
    ...
    def evaluate(self, X):
        return self.H(self.h(X, training = False), training = False)
